...
FP="$CACHE/${NMS_PROFILE}.fingerprint"   # local fingerprint cache

escape_sql() { printf "%s" "$1" | sed "s/'/''/g"; }

# --- preflight (decode → maybe skip import) ---
...
# --- compute fingerprint (BEFORE we do any extraction/import) ---
...
SLOTS_ESC=$(printf "%s" "$SLOTS" | sed "s/'/''/g")

# 1) local fingerprint skip
if [[ -f "$FP" ]]; then
  PREV_FP="$(<"$FP")"
...
# 2) DB fingerprint skip (only if not already skipping)
if [[ $SKIP_IMPORT -eq 0 ]]; then
  DB_SHA=$(
    MYSQL_PWD="$NMS_DB_PASS" mariadb --local-infile=1 \
      -h "$NMS_DB_HOST" -P "$NMS_DB_PORT" -u "$NMS_DB_USER" -D "$NMS_DB_NAME" -N -B \
      -e "SELECT json_sha256 FROM nms_snapshots
...
if [[ $SKIP_IMPORT -eq 0 ]]; then
  ...
  cat <<SQL | MYSQL_PWD="$NMS_DB_PASS" mariadb --local-infile=1 \
    -h "$NMS_DB_HOST" -P "$NMS_DB_PORT" -u "$NMS_DB_USER" -D "$NMS_DB_NAME" -N -B
...
SQL
  ...
  SNAP=$(
    MYSQL_PWD="$NMS_DB_PASS" mariadb --local-infile=1 \
      -h "$NMS_DB_HOST" -P "$NMS_DB_PORT" -u "$NMS_DB_USER" -D "$NMS_DB_NAME" -N -B \
      -e "SELECT snapshot_id FROM nms_snapshots
          WHERE source_path='${HG_ESC}' AND source_mtime='${HG_MTIME}'
          ORDER BY snapshot_id DESC LIMIT 1;"
  )
  echo "[dev] snapshot_id = ${SNAP:-?}"

  # replace rows for this snapshot
  MYSQL_PWD="$NMS_DB_PASS" mariadb --local-infile=1 \
    -h "$NMS_DB_HOST" -P "$NMS_DB_PORT" -u "$NMS_DB_USER" -D "$NMS_DB_NAME" -N -B \
    -e "DELETE FROM nms_items WHERE snapshot_id=${SNAP:-0};"

  MYSQL_PWD="$NMS_DB_PASS" mariadb --local-infile=1 \
    -h "$NMS_DB_HOST" -P "$NMS_DB_PORT" -u "$NMS_DB_USER" -D "$NMS_DB_NAME" -N -B \
    -e "LOAD DATA LOCAL INFILE '${SLOTS_ESC}'
  ...
  MYSQL_PWD="$NMS_DB_PASS" mariadb --local-infile=1 \
    -h "$NMS_DB_HOST" -P "$NMS_DB_PORT" -u "$NMS_DB_USER" -D "$NMS_DB_NAME" -N -B \
    -e "SELECT COUNT(*) AS rows_loaded FROM nms_items WHERE snapshot_id=${SNAP:-0};"

  # write local fingerprint
  printf '%s\n' "$CUR_FP" > "$FP"
fi
